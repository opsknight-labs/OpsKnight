...
Backup evidence starts with the [recovery procedure](../../v1.5/deployment/backup-restore.md), [restore workflow](../../../.github/workflows/enterprise-readiness.yml) and [verifier](../../../deploy/scripts/drills/verify-backup-restore.sh). Download actual drill output and record the run and commit. The workflow retains its reports for 90 days; that is not a production recovery guarantee. Operators must preserve their own restoration results, recovery times, backup age, key availability and scheduled drill records.
