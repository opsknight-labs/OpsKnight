...
The feed is deliberately level-triggered: consumers need to know that state changed, not replay every mutation. A bounded tail is retained, so disconnected replicas and newly connected clients load a current snapshot instead of replaying history. Thirty-second SSE heartbeats keep proxies alive; authorization is revalidated independently every minute. If the change-clock query temporarily fails, the stream stays connected, applies bounded retry backoff, and reports the failure through logs, deep health, and Prometheus.
