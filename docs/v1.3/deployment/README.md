...
The container entrypoint runs `prisma migrate deploy` before starting the server. It retries/recovery-attempts migration failures, but if the schema still cannot be migrated the container exits non-zero. OpsKnight does not intentionally serve against an unknown schema.
