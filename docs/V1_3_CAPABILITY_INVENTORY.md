...
- **Unsupported** means an older document or index claimed behavior that v1.3 does not provide; the claim must be removed or called out as unavailable.
- A phase cannot close while any row assigned to it remains **Add** or **Revise**.

Evidence is based on v1.3 application routes, components, Prisma schema, deployment manifests, tests, and repeatable product workflows. Maintainers should add a PR link or test result to a row when verification needs more context than a source path.
...
| -------------------------------------------------- | ---------------------------- | ------------------------------------------------------ | -------- | ------------------------------------------------------ | ------------------------ | ----- |
| Compose evaluation path                            | Evaluator                    | `docker-compose.yml`; getting-started flow             | Complete | `v1.3/getting-started/README.md`                       | Golden path              | 1     |
| First-week configuration                           | New admin                    | Application setup routes                               | Complete | `v1.3/getting-started/first-steps.md`                  | Guide                    | 1     |
| Dashboard command center                           | Responder                    | `src/app/(app)/page.tsx`; dashboard components         | Complete | `v1.3/core-concepts/dashboard.md`                      | Guide and limits         | 2     |
| Desktop navigation and global search               | Signed-in user               | `Sidebar`, `SidebarSearch`, `/api/search`               | Complete | `v1.3/core-concepts/navigation-search-notifications.md` | Task guide and limits    | 2     |
| Personal in-app notification inbox                 | Signed-in user               | `TopbarNotifications`; notification routes/stream      | Complete | `v1.3/core-concepts/navigation-search-notifications.md` | Task guide and limits    | 2, 3  |
| Incident create, triage, bulk actions, lifecycle   | Responder                    | incident routes, actions, components, `Incident` model | Complete | `v1.3/core-concepts/incidents.md`                      | Full guide               | 2     |
| Incident notes, tags, watchers, timeline           | Responder                    | incident detail components and models                  | Complete | `v1.3/core-concepts/incidents.md#add-response-context` | Section                  | 2     |
| Incident custom fields                             | Admin, responder             | settings custom-fields route; `IncidentCustomFields`   | Complete | `v1.3/core-concepts/incidents.md`; admin guide         | Section plus admin setup | 2, 5  |
| Incident templates                                 | Responder, admin             | templates route and actions                            | Complete | `v1.3/core-concepts/incident-templates.md`             | Guide                    | 2     |
| Service directory, ownership, health, SLA          | Admin, responder             | service routes/components; `Service` model             | Complete | `v1.3/core-concepts/services.md`                       | Full guide               | 2     |
| Teams, membership, roles, notification preference  | Admin, manager               | team routes/components; `TeamMember` model             | Complete | `v1.3/core-concepts/teams.md`                          | Full guide               | 2     |
| Users, invitations, roles, status, profile         | Admin, user                  | user/settings routes; `User` model                     | Complete | `v1.3/core-concepts/users.md`                          | Full guide               | 2, 5  |
| On-call schedules, layers, rotations, overrides    | On-call manager              | schedule routes/actions/models                         | Complete | `v1.3/core-concepts/schedules.md`                      | Full guide               | 2     |
| Escalation policies: user, team, schedule targets  | On-call manager              | policy routes/actions; `EscalationRule`                | Complete | `v1.3/core-concepts/escalation-policies.md`            | Full guide               | 2     |
| Urgency, priority, event severity mapping          | Integration owner, responder | inbound adapters; incident schema                      | Complete | `v1.3/core-concepts/urgency-mapping.md`                | Reference                | 2, 3  |
| Analytics filters, SLA metrics, CSV export         | Manager                      | analytics page/export route; SLA libraries             | Complete | `v1.3/core-concepts/analytics.md`                      | Guide and limits         | 2     |
| Custom reports and dashboards                      | Manager                      | reports routes and dashboard API/model                 | Complete | `v1.3/core-concepts/reports-dashboards.md`             | Guide and limits         | 2     |
| Postmortem authoring and publication               | Responder, manager           | postmortem routes/actions/model                        | Complete | `v1.3/core-concepts/postmortems.md`                    | Guide and limits         | 2     |
| Action-item board and Jira sync                    | Responder, manager           | action-items route/actions/models                      | Complete | `v1.3/core-concepts/action-items.md`                   | Guide                    | 2     |
| Event log and Events test console                  | Integration owner            | event routes and page                                  | Complete | `v1.3/core-concepts/event-logs.md`                     | Guide and limits         | 2     |
| Public status page configuration                   | Admin, communicator          | status-page settings/routes/models                     | Complete | `v1.3/core-concepts/status-page.md`                    | Full guide               | 2     |
| Status services, incidents, announcements, metrics | Communicator                 | status-page components and APIs                        | Complete | `v1.3/core-concepts/status-page.md`                    | Task sections            | 2     |
| Status subscribers and outbound webhooks           | Admin, communicator          | subscriber/webhook routes and libraries                | Complete | `v1.3/core-concepts/status-page.md`                    | Setup and operations     | 2     |
...
| Administrator operational health center      | Admin, operator             | database capacity, scheduler/jobs, paging, delivery, SLA performance, runtime, configuration, version | Complete | `v1.3/administration/health-center.md` | Status, limits, response | 4, 5 |
...
| Capability                                       | Audience               | Evidence                                           | State       | Documentation destination   | Depth               | Phase |
| ------------------------------------------------ | ---------------------- | -------------------------------------------------- | ----------- | --------------------------- | ------------------- | ----- |
| Events API trigger, acknowledge, resolve         | API consumer           | `/api/events`; schemas/tests                       | Complete    | `v1.3/api/events.md`        | Published contract  | 6     |
| Incidents API published operations               | API consumer           | published incident routes/tests                    | Complete    | `v1.3/api/incidents.md`     | Published contract  | 6     |
| PostgreSQL-backed API rate limiting              | API consumer, operator | rate-limit library/schema                          | Complete    | `v1.3/api/rate-limiting.md` | Contract/operations | 6     |
| CLI commands and configuration                   | Operator               | `scripts/OpsKnight.mjs`; package scripts           | Complete    | `v1.3/api/cli.md`           | Command reference   | 6     |
