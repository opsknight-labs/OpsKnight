...
const resolveMicrosoftTeamsUser = vi.fn<
  () => Promise<{ linkId: string; userId: string; displayName: string } | null>
>(async () => ({ linkId: 'link-1', userId: 'user-1', displayName: 'Alice' }));
...
const enqueueChatOpsIntent = vi.fn(
  async (input: { payload: Record<string, unknown>; payloadDigest?: string }) => {
    persistedIntentPayload = input.payload;
    capturedPayloadDigest = input.payloadDigest;
    return { id: 'intent-1', duplicate: false };
  }
);
const processInlineChatOpsIntent = vi.fn(
  async (
    _id: string,
    execute: (input: { intentId: string; payload: Record<string, unknown> }) => Promise<unknown>
  ) => execute({ intentId: 'intent-1', payload: persistedIntentPayload })
);
...
const prismaMock = {
  microsoftTeamsConfig: {
    findFirst: vi.fn(async () => ({ id: 'config', enabled: true, interactiveEnabled: true })),
  },
  microsoftTeamsDestination: { findUnique: vi.fn(async () => destination) },
  incident: {
    findUnique: vi.fn(async (args: { select?: unknown }) =>
      args.select ? { assignee: null, watchers: [] } : incident
    ),
  },
...
vi.mock('@/lib/chatops/incident-capabilities', () => ({
  getIncidentChatOpsCapabilities: vi.fn(async () => ({ canRead: true })),
}));
vi.mock('@/lib/microsoft-teams/identity', () => ({
  resolveMicrosoftTeamsUser,
  createMicrosoftTeamsIdentityChallenge,
}));
...
function activity(overrides: Record<string, unknown> = {}) {
  return {
    id: 'invoke-1',
    replyToId: 'message-1',
    from: { id: 'teams-user', aadObjectId: 'aad-1', name: 'Alice' },
...
describe('Microsoft Teams invoke adapter', () => {
  beforeEach(() => {
    ...
    resolveMicrosoftTeamsUser.mockResolvedValue({
      linkId: 'link-1',
      userId: 'user-1',
      displayName: 'Alice',
    });
  ...
  it('binds a valid click and dispatches through the provider-neutral command', async () => {
    ...
    expect(executeChatOpsCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        provider: 'MICROSOFT_TEAMS',
        command: { kind: 'ACKNOWLEDGE', incidentId: 'inc-1' },
        idempotency: {
          key: 'intent-1',
          principalId: 'chatops:microsoft-teams:tenant-1:teams-user',
        },
      })
    );
  });
  ...
  it('rejects a cross-channel or stale-generation card before identity or mutation', async () => {
    const { handleMicrosoftTeamsAdaptiveCardAction } = await import('@/lib/microsoft-teams/invoke');
    const wrongChannel = activity({
      channelData: { tenant: { id: 'tenant-1' }, team: { id: 'team-1' }, channel: { id: 'evil' } },
    });
    await expect(
      handleMicrosoftTeamsAdaptiveCardAction({
        activity: wrongChannel,
        verifiedTenantId: 'tenant-1',
      })
      ...
      activity({
        value: {
          action: {
            type: 'Action.Execute',
            verb: 'opsknight.incident.ack',
            data: { v: 2, incidentId: 'inc-1', destinationId: 'dest-1', messageGeneration: 2 },
          },
        },
      }),
    ],
  ...
  it('rejects clicks after destination disablement', async () => {
    ...
    const { handleMicrosoftTeamsAdaptiveCardAction } = await import('@/lib/microsoft-teams/invoke');
    await expect(
      handleMicrosoftTeamsAdaptiveCardAction({ activity: activity(), verifiedTenantId: 'tenant-1' })
  ...
  it('persists a fixed snooze deadline and reuses it during intent execution', async () => {
    ...
    const { handleMicrosoftTeamsAdaptiveCardAction } = await import('@/lib/microsoft-teams/invoke');
    const snooze = activity({
      value: {
        action: {
          type: 'Action.Execute',
          verb: 'opsknight.incident.snooze',
          data: {
            v: 2,
            incidentId: 'inc-1',
    ...
    await expect(
      handleMicrosoftTeamsAdaptiveCardAction({ activity: snooze, verifiedTenantId: 'tenant-1' })
    ...
    expect(executeChatOpsCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        command: expect.objectContaining({
          kind: 'SNOOZE',
          snoozedUntil: new Date('2026-09-13T10:30:00.000Z'),
        }),
      })
    );
