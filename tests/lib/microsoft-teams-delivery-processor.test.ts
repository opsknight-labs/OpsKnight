...
let ownerOperation: {
  ...
  resultPayload: { createAttempted: true },
};
...
const incident = {
  ...
  acknowledgedAt: new Date('2026-01-01T00:01:00Z'),
  resolvedAt: new Date('2026-01-01T00:02:00Z'),
...
const prismaMock = {
      ...
      select?.status
        ...
        : {
            ...
            requestPayload: {
              ...
              eventType: 'resolved',
              incidentUpdatedAt: incident.resolvedAt.toISOString(),
            },
            resultPayload: currentResultPayload,
          }
    ),
  },
  ...
  service: {
    findUnique: vi.fn(async () => ({
      serviceNotificationChannels: ['MICROSOFT_TEAMS'],
  ...
  microsoftTeamsIncidentMessage: {
    ...
    updateMany: vi.fn(async () => ({ count: ledgerUpdateCount })),
  },
  $transaction: vi.fn(async (callback: (tx: unknown) => Promise<unknown>) =>
    callback({
      ...
      externalOperation: {
        findUnique: vi.fn(async () => ownerOperation),
...
vi.mock('@/lib/provider-admission', () => ({
  ...
  releaseProviderConcurrency: vi.fn(async () => undefined),
  deferProviderAdmission: vi.fn(async () => undefined),
}));
...
vi.mock('@/lib/microsoft-teams/provider', () => ({
  microsoftTeamsChatProvider: {
    sendIncidentCard,
    createIncidentCard,
    updateIncidentCard,
    recoverIncidentCard,
...
describe('processMicrosoftTeamsOperation create fence', () => {
  ...
  it('does not issue update or recovery create while a previous create is ambiguous', async () => {
    ...
    await expect(processMicrosoftTeamsOperation('op-late')).rejects.toThrow(
      'requires create reconciliation'
    );
  ...
  it('defers behind a live CREATING owner without issuing a provider call', async () => {
    ...
    expect(recoverIncidentCard).not.toHaveBeenCalled();
    expect(operationUpdates).toContainEqual(
      expect.objectContaining({ status: 'PENDING', attempts: { decrement: 1 } })
    );
  });
  ...
  it('safely resumes a same-operation CREATING fence when the worker died before provider I/O', async () => {
    ...
    ledgerUpdateCount = 1;
    createIncidentCard.mockImplementationOnce(
      async ({ beforeCreateAttempt }: { beforeCreateAttempt: () => Promise<void> }) => {
...
describe('processMicrosoftTeamsOperation admission & concurrency classification', () => {
  beforeEach(() => {
    operationUpdates.length = 0;
    currentResultPayload = null;
    vi.clearAllMocks();
    vi.mocked(acquireProviderAdmission).mockResolvedValue({ allowed: true });
    vi.mocked(acquireProviderConcurrency).mockResolvedValue({ allowed: true, leaseKey: 'lease' });
  });

  it('passes TRANSACTIONAL trafficClass for resolved events', async () => {
    ...
    const { processMicrosoftTeamsOperation } = await import('@/lib/microsoft-teams/delivery');
    try {
      await processMicrosoftTeamsOperation('op-late');
    } catch {}

    expect(acquireProviderAdmission).toHaveBeenCalledWith(
      'MICROSOFT_TEAMS',
      'tenant:tenant-1',
      expect.any(Date),
      'TRANSACTIONAL'
    );
    expect(acquireProviderConcurrency).toHaveBeenCalledWith(
      'MICROSOFT_TEAMS',
      'tenant:tenant-1',
      expect.any(Date),
      'TRANSACTIONAL'
  ...
  it('passes CRITICAL trafficClass for high-urgency triggered events', async () => {
    prismaMock.externalOperation.findUnique.mockImplementationOnce(
        ...
        select?.status
          ...
          : {
              ...
              requestPayload: {
                destinationId: 'dest-1',
                eventType: 'triggered',
                incidentUpdatedAt: incident.createdAt.toISOString(),
                escalationGeneration: 0,
              },
              resultPayload: null,
            }
    );
    prismaMock.incident.findUnique.mockImplementationOnce(async () => ({
      ...incident,
      status: 'OPEN',
      urgency: 'HIGH',
      priority: 'P1',
    ...
    const { processMicrosoftTeamsOperation } = await import('@/lib/microsoft-teams/delivery');
    try {
      await processMicrosoftTeamsOperation('op-crit');
    } catch {}
  ...
  it('preserves CONTROL_PLANE_UNAVAILABLE failureCode on admission rejection', async () => {
    Object.assign(ledger, {
      createState: 'NONE',
      createOperationId: null,
      messageId: 'msg-1',
    ...
    vi.mocked(acquireProviderAdmission).mockResolvedValueOnce({
      allowed: false,
    ...
    const { processMicrosoftTeamsOperation } = await import('@/lib/microsoft-teams/delivery');
    const err = await processMicrosoftTeamsOperation('op-late').catch(e => e);
    ...
    expect((err as AppError).message).toContain('control plane is degraded');
    expect(operationUpdates).toContainEqual(
      expect.objectContaining({
        status: 'PENDING',
        attempts: { decrement: 1 },
        lastError: expect.stringContaining('Teams admission CONTROL_PLANE_UNAVAILABLE'),
      })
    );
  });

  it('preserves CONTROL_PLANE_UNAVAILABLE failureCode on concurrency rejection', async () => {
    Object.assign(ledger, {
      createState: 'NONE',
      createOperationId: null,
      messageId: 'msg-1',
    ...
    vi.mocked(acquireProviderConcurrency).mockResolvedValueOnce({
      allowed: false,
      reason: 'CONTROL_PLANE_UNAVAILABLE',
      retryAt: new Date(Date.now() + 5_000),
      cause: 'Mocked concurrency control plane failure',
    });
    ...
    expect(operationUpdates).toContainEqual(
      expect.objectContaining({
        status: 'PENDING',
        attempts: { decrement: 1 },
        lastError: expect.stringContaining('Teams concurrency CONTROL_PLANE_UNAVAILABLE'),
      })
    );
  });
});
