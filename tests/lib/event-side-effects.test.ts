...
describe('event durable side effects', () => {
  ...
  it('lets notification-provider failure escape so the durable job retries', async () => {
    executeEscalationMock.mockResolvedValue({ escalated: false, reason: 'No escalation policy configured' });
    sendIncidentNotificationsMock.mockRejectedValue(new Error('provider unavailable'));
    await expect(processEventSideEffect(payload('TRIGGER_ESCALATION_NOTIFICATIONS'))).rejects.toThrow('provider unavailable');
  ...
  it('retries an explicit notification failure result', async () => {
    ...
    await expect(processEventSideEffect(payload('TRIGGER_ESCALATION_NOTIFICATIONS'))).rejects.toThrow('push failed');
  });
  ...
  it.each([
    ...
    await expect(processEventSideEffect(payload('TRIGGER_ESCALATION_NOTIFICATIONS'))).resolves.toBeUndefined();
  });

  it('retries a typed transient notification provider failure', async () => {
    executeEscalationMock.mockResolvedValue({ escalated: false, reason: 'No escalation policy configured' });
    sendIncidentNotificationsMock.mockResolvedValue({ success: false, outcome: 'RETRYABLE_FAILURE', errors: ['provider unavailable'] });
    await expect(processEventSideEffect(payload('TRIGGER_ESCALATION_NOTIFICATIONS'))).rejects.toThrow('provider unavailable');
  });

  it('completes a typed permanent notification failure without endless retries', async () => {
    executeEscalationMock.mockResolvedValue({ escalated: false, reason: 'No escalation policy configured' });
    sendIncidentNotificationsMock.mockResolvedValue({ success: false, outcome: 'PERMANENT_FAILURE', errors: ['provider credentials are invalid'] });
    await expect(processEventSideEffect(payload('TRIGGER_ESCALATION_NOTIFICATIONS'))).resolves.toBeUndefined();
  });
  ...
  it('keeps team assignment intent and recipients distinct from a generic update', async () => {
    ...
    expect(sendIncidentNotificationsMock).toHaveBeenCalledWith('inc-1', 'updated', [], undefined, { intent: 'ASSIGNED_TO_TEAM' });
  });
  ...
  it('keeps created webhook lifecycle state even if the incident has since resolved', async () => {
    prismaMock.incident.findUnique.mockResolvedValue({
      id: 'inc-1', title: 'Incident', description: null, status: 'RESOLVED', urgency: 'HIGH', priority: 'P1', serviceId: 'svc-1',
      service: { id: 'svc-1', name: 'Service' }, assignee: null, createdAt: new Date('2026-08-26T12:00:00Z'), acknowledgedAt: null, resolvedAt: new Date('2026-08-26T12:01:00Z'),
    });
    triggerWebhooksForServiceMock.mockResolvedValue(undefined);
    await processEventSideEffect(payload('TRIGGER_WEBHOOK', 'WEBHOOK'));
    expect(triggerWebhooksForServiceMock).toHaveBeenCalledWith('svc-1', 'incident.created', expect.objectContaining({ status: 'OPEN' }));
  });

  it('retries partial webhook delivery failures', async () => {
    prismaMock.incident.findUnique.mockResolvedValue({
      id: 'inc-1', title: 'Incident', description: null, status: 'OPEN', urgency: 'HIGH', priority: 'P1', serviceId: 'svc-1',
      service: { id: 'svc-1', name: 'Service' }, assignee: null, createdAt: new Date('2026-08-26T12:00:00Z'), acknowledgedAt: null, resolvedAt: null,
    });
    triggerWebhooksForServiceMock.mockResolvedValue({ attempted: 2, failed: 1 });
    await expect(processEventSideEffect(payload('TRIGGER_WEBHOOK', 'WEBHOOK'))).rejects.toThrow('failed for 1 delivery target');
  });

  it('replays the queued lifecycle state instead of leaking a newer current state into webhooks', async () => {
    prismaMock.incident.findUnique.mockResolvedValue({
      id: 'inc-1', title: 'Incident', description: null, status: 'RESOLVED', urgency: 'HIGH', priority: 'P1', visibility: 'PUBLIC', serviceId: 'svc-1',
      service: { id: 'svc-1', name: 'Service' }, assignee: null, createdAt: new Date('2026-08-28T06:00:00.000Z'), acknowledgedAt: new Date('2026-08-28T06:01:00.000Z'), resolvedAt: new Date('2026-08-28T06:02:00.000Z'),
    });
    triggerWebhooksForServiceMock.mockResolvedValue(undefined);
    await processEventSideEffect(payload('LIFECYCLE_WEBHOOK', 'WEBHOOK', {
      command: 'ACKNOWLEDGE', source: 'REST_API', previousStatus: 'OPEN', status: 'ACKNOWLEDGED', transitionAt: '2026-08-28T06:01:00.000Z', snoozedUntil: null,
    }));
    expect(triggerWebhooksForServiceMock).toHaveBeenCalledWith('svc-1', 'incident.acknowledged', expect.objectContaining({ id: 'inc-1', status: 'ACKNOWLEDGED' }));
  });
  ...
  it('lets lifecycle notification failures escape to the durable job retry path', async () => {
    ...
    await expect(processEventSideEffect(payload('LIFECYCLE_USER_NOTIFICATION', 'NOTIFICATION', {
      command: 'RESOLVE', source: 'WEB', previousStatus: 'ACKNOWLEDGED', status: 'RESOLVED', transitionAt: '2026-08-28T06:02:00.000Z', snoozedUntil: null,
  ...
  it('recreates an archived war-room on reopen before syncing the OPEN state', async () => {
    ...
    createIncidentWarRoomMock.mockResolvedValue({ success: true, channelId: 'C123', channelName: 'incident-123' });
    postWarRoomUpdateMock.mockResolvedValue({ success: true });
    updateWarRoomTopicMock.mockResolvedValue({ success: true });
    await processEventSideEffect(payload('LIFECYCLE_WAR_ROOM_ENSURE', 'WAR_ROOM', {
      command: 'REOPEN', source: 'WEB', previousStatus: 'RESOLVED', status: 'OPEN', transitionAt: '2026-08-28T07:00:00.000Z', snoozedUntil: null,
    }));
  ...
  it('does not archive a war-room when an old resolve effect runs after reopen', async () => {
    ...
    await processEventSideEffect(payload('LIFECYCLE_WAR_ROOM_ARCHIVE', 'WAR_ROOM', {
      command: 'RESOLVE', source: 'WEB', previousStatus: 'ACKNOWLEDGED', status: 'RESOLVED', transitionAt: '2026-08-28T06:02:00.000Z', snoozedUntil: null,
    }));
  ...
  it('does not let an older resolve archive a newer resolve generation', async () => {
    prismaMock.incident.findUnique.mockResolvedValue({ status: 'RESOLVED', resolvedAt: new Date('2026-08-28T07:10:00.000Z') });
    await processEventSideEffect(payload('LIFECYCLE_WAR_ROOM_ARCHIVE', 'WAR_ROOM', {
      command: 'RESOLVE', source: 'WEB', previousStatus: 'ACKNOWLEDGED', status: 'RESOLVED', transitionAt: '2026-08-28T06:02:00.000Z', snoozedUntil: null,
    }));
    expect(archiveWarRoomChannelMock).not.toHaveBeenCalled();
  ...
  it('archives the war-room only for the still-current resolve transition', async () => {
    prismaMock.incident.findUnique.mockResolvedValue({ status: 'RESOLVED', resolvedAt: new Date('2026-08-28T06:02:00.000Z') });
  ...
  it('rejects an unknown effect instead of silently completing it', async () => {
    const invalid = { ...payload('ACK_SLACK', 'SLACK'), effect: 'UNKNOWN_EFFECT' } as unknown as EventSideEffectPayload;
