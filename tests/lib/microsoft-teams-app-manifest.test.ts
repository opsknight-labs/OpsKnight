...
describe('Microsoft Teams app manifest', () => {
  it('exposes the bot messaging endpoint via helper (not manifest)', () => {
    expect(getMicrosoftTeamsBotMessagingEndpoint(BASE)).toBe(
      `${BASE}/api/microsoft-teams/messages`
    );
  });
  ...
  it('tolerates a base URL with a trailing slash', () => {
    ...
    const mWithUri = buildMicrosoftTeamsAppManifest({
      appUrl: `${BASE}/`,
      botId: BOT_ID,
      applicationIdUri: `api://${new URL(BASE).hostname}/${BOT_ID}`,
    });
    expect((mWithUri as Record<string, unknown>).webApplicationInfo).toBeDefined();
    expect((mWithUri.webApplicationInfo as { resource: string }).resource).toContain(
  ...
  it('keeps destination delivery minimal and makes war-room RSC opt-in', () => {
    ...
    expect([...MICROSOFT_TEAMS_REQUIRED_RSC_PERMISSIONS]).toEqual(['ChannelSettings.Read.Group']);
    expect(MICROSOFT_TEAMS_OPTIONAL_RSC_PERMISSIONS).toContain('TeamSettings.Read.Group');
    expect(MICROSOFT_TEAMS_OPTIONAL_RSC_PERMISSIONS).toEqual(
  ...
  it('declares RSC permissions as Application-scoped in the manifest', () => {
    ...
    const m = buildMicrosoftTeamsAppManifest({ appUrl: BASE, botId: BOT_ID });
    const names = (m.authorization.permissions.resourceSpecific as Array<{ name: string }>).map(
      p => p.name
    );
    ...
    const mFull = buildMicrosoftTeamsAppManifest({
      appUrl: BASE,
      botId: BOT_ID,
      includeOptionalPermissions: true,
    });
    const fullNames = (
      mFull.authorization.permissions.resourceSpecific as Array<{ name: string }>
    ).map(p => p.name);
    expect(fullNames).toContain('TeamSettings.Read.Group');
    expect(fullNames).not.toContain('Channel.Create.Group');
    const warRoom = buildMicrosoftTeamsAppManifest({
      appUrl: BASE,
      botId: BOT_ID,
      includeWarRoomPermissions: true,
    });
    const warRoomNames = (
      warRoom.authorization.permissions.resourceSpecific as Array<{ name: string }>
    ).map(p => p.name);
    ...
    const all = buildMicrosoftTeamsAppManifest({
      appUrl: BASE,
      botId: BOT_ID,
      includeTeamSettingsPermissions: true,
      includeWarRoomPermissions: true,
    });
    const allNames = (
      all.authorization.permissions.resourceSpecific as Array<{ name: string }>
    ).map(p => p.name);
    for (const perm of [
      ...MICROSOFT_TEAMS_REQUIRED_RSC_PERMISSIONS,
    ...
    const collaborationNames = (
      collaboration.authorization.permissions.resourceSpecific as Array<{ name: string }>
    ).map(p => p.name);
    expect(collaborationNames).toEqual(
      expect.arrayContaining([
        'Channel.Create.Group',
        'ChannelSettings.ReadWrite.Group',
        'TeamMember.Read.Group',
  ...
  it('always emits webApplicationInfo and permits an Entra Application ID URI override', () => {
    ...
    expect(mDefault.webApplicationInfo).toEqual({
      id: BOT_ID,
      resource: `api://${new URL(BASE).hostname}/${BOT_ID}`,
    });
  ...
  it('sanitizes extraValidDomains and strictly strips prohibited Microsoft domains', () => {
    const m = buildMicrosoftTeamsAppManifest({
      appUrl: BASE,
      botId: BOT_ID,
      extraValidDomains: [
        ...
        'azure.com',
        'office.com',
        'https://opssentinal.com/path?query=1',
        '*.opssentinal.com',
        '  OPsKnight.com  ',
        '*.opsknight.com',
      ],
    ...
    expect(m.validDomains).toContain('opsknight.example.com');
    expect(m.validDomains).toContain('opssentinal.com');
    expect(m.validDomains).toContain('*.opssentinal.com');
    expect(m.validDomains).toContain('opsknight.com');
    expect(m.validDomains).toContain('*.opsknight.com');
    expect(m.validDomains).not.toContain('teams.microsoft.com');
    expect(m.validDomains).not.toContain('*.teams.microsoft.com');
    expect(m.validDomains).not.toContain('microsoft.com');
    expect(m.validDomains).not.toContain('*.microsoft.com');
    expect(m.validDomains).not.toContain('azure.com');
    expect(m.validDomains).not.toContain('office.com');
  });
});
...
describe('findMissingRequiredRscPermissions', () => {
  it('reports nothing when every required permission is granted', () => {
    expect(
      findMissingRequiredRscPermissions([...MICROSOFT_TEAMS_REQUIRED_RSC_PERMISSIONS])
    ).toEqual([]);
  });

  it('reports the missing required permission', () => {
    expect(findMissingRequiredRscPermissions([])).toEqual(['ChannelSettings.Read.Group']);
    expect(findMissingRequiredRscPermissions(['TeamSettings.Read.Group'])).toEqual([
      'ChannelSettings.Read.Group',
    ]);
  });

  it('treats absent/empty grant list as everything missing', () => {
    expect(findMissingRequiredRscPermissions(undefined)).toEqual([
      ...MICROSOFT_TEAMS_REQUIRED_RSC_PERMISSIONS,
    ]);
    expect(findMissingRequiredRscPermissions([])).toEqual([
      ...MICROSOFT_TEAMS_REQUIRED_RSC_PERMISSIONS,
    ]);
    expect(findMissingRequiredRscPermissions(null as unknown as string[])).toEqual([
      ...MICROSOFT_TEAMS_REQUIRED_RSC_PERMISSIONS,
    ]);
  });
