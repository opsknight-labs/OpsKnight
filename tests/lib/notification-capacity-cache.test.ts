...
describe('notification capacity cache hardening', () => {
  ...
  it('remains bounded under webhook-origin cardinality (LRU, maxEntries)', () => {
    ...
    for (let i = 0; i < 600; i++) {
      capacityCache.set(`WEBHOOK:https://hooks.example.com/${i}`, { v: i } as unknown, now, 5000);
    }
    // Must be bounded — not unbounded growth
    expect(capacityCache._size()).toBeLessThanOrEqual(512);
    expect(capacityCache._size()).toBeGreaterThan(0);
  });
  ...
  it('cached null is distinguishable from miss (negative cache semantics on runtime)', () => {
    ...
    runtimeCache.set('runtime', null as unknown, now, 5000);
  ...
  it('expired entries are not returned (TTL expiry)', () => {
    const t0 = 5000;
    capacityCache.set('EMAIL:default', { v: 1 } as unknown, t0, 5000);
  ...
  it('invalidate(key) only drops that key; invalidate() clears all', () => {
    capacityCache.set('EMAIL:default', { a: 1 } as unknown, 10_000, 5000);
    capacityCache.set('SMS:default', { a: 2 } as unknown, 10_000, 5000);
  ...
  it('sweepExpired prevents gradual growth from expired entries left behind by churn', () => {
    ...
    for (let i = 0; i < 510; i++) {
      capacityCache.set(`k:${i}`, { v: i } as unknown, t0, 1000);
    }
    expect(capacityCache._size()).toBe(510);
    // After TTL, adding a new key should sweep expired and stay bounded
    capacityCache.set('k:new', { v: 999 } as unknown, t0 + 2000, 5000);
    // Expired 510 should have been swept before eviction logic
    expect(capacityCache._size()).toBeLessThanOrEqual(10);
  });
});
