...
import { PASSWORD_MAX_LENGTH, PASSWORD_MAX_UTF8_BYTES, PASSWORD_MIN_LENGTH } from '@/lib/passwords';
...
describe('Security Components', () => {
  ...
  describe('PasswordStrength', () => {
    it('renders the centralized passphrase requirements without composition rules', () => {
      ...
      expect(
        screen.getByText(`No more than ${PASSWORD_MAX_LENGTH} characters`)
      ).toBeInTheDocument();
      expect(
  ...
  describe('ActiveSessionsSection', () => {
    it('renders current browser activity and the account-wide revocation action', async () => {
      ...
      expect(
        await screen.findByRole('button', { name: /Revoke all sessions/i })
      ).toBeInTheDocument();
      expect(
        screen.getByText(/No active signed-in sessions found/i) ||
