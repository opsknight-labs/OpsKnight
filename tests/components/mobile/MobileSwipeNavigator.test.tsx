...
describe('MobileSwipeNavigator', () => {
  ...
  const swipe = (element: HTMLElement, fromX: number, fromY: number, toX: number, toY: number) => {
    const PointerEventCtor = typeof PointerEvent === 'undefined' ? MouseEvent : PointerEvent;
    ...
    element.dispatchEvent(
      new PointerEventCtor('pointerup', { ...init, clientX: toX, clientY: toY } as PointerEventInit)
    );
  };
  ...
  it('navigates to the next tab on left swipe', () => {
    ...
    expect(mockPush).toHaveBeenCalledWith('/m/schedules');
  });
