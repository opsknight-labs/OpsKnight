...
async function validateSlackWarRoomProjectionAuthority(room: {
  ...
  if (!config?.enabled)
    return {
      ...
      message: 'ChatOps war-room configuration was disabled.',
    };
  if (!integration)
    return {
      allowed: false,
      code: 'WAR_ROOM_AUTHORITY_REVOKED',
      message: 'Slack workspace installation was disabled.',
    };
  return { allowed: true };
}
  ...
  if (!botToken) {
    await prisma.incidentWarRoom.updateMany({
      ...
      data: {
        ...
        lastError: 'No Slack bot token configured for projection.',
        projectionLeaseToken: null,
        projectionLeaseExpiresAt: null,
      },
    });
  ...
  if (!room.commandMessageId) {
    ...
    if (!created.ok) {
      ...
      await prisma.incidentWarRoom.updateMany({
        ...
        data: {
          health: 'DEGRADED',
          lastErrorCode: definitePrePostFailure
            ? (created.error?.toUpperCase() ?? 'CARD_CREATE_FAILED')
            : 'AMBIGUOUS_CARD_CREATE',
          lastError: created.error ?? 'Slack card create failed',
        },
      });
    ...
    const ts =
      (created as unknown as { ts?: string; message?: { ts?: string } }).ts ??
    ...
    if (ts) {
      ...
      await prisma.incidentWarRoom.updateMany({
        ...
        data: {
          ...
          lastError:
            'Slack card create returned success without a message identifier; canonical card outcome is ambiguous.',
          projectionLeaseToken: null,
          projectionLeaseExpiresAt: null,
        },
      });
    ...
    if (!updated.ok) {
      ...
      await prisma.incidentWarRoom.updateMany({
        ...
        data: {
          health: 'DEGRADED',
          lastErrorCode: 'CARD_UPDATE_FAILED',
          lastError: updated.error ?? 'Slack card update failed',
        },
      });
      if (retryable) {
        await prisma.incidentWarRoom.updateMany({
