...
export async function sendNotificationIntentPush(
  ...
  const timeLabel = formatPushTimestamp(
    new Date(snapshot.eventAt),
    getUserTimeZone(user ?? undefined)
  );
  const owner =
    snapshot.assignee?.name || snapshot.assignee?.email || snapshot.team?.name || 'Unassigned';
  const escalation = snapshot.escalationLevel;
  const eventLabel = escalation
  ...
  if ((eventType === 'triggered' || escalation) && snapshot.urgency === 'HIGH')
    body += '\n🚨 Urgent Action Required';
  if (displayMessage && !displayMessage.includes(snapshot.title))
