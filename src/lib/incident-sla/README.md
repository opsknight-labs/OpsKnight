...
Both phases breach only when elapsed time is strictly greater than the target. A source recovery before the ACK deadline makes acknowledgement `NOT_REQUIRED`; source recovery after the deadline and manual or unknown resolution without ACK remain breaches. Snooze and suppression stop the canonical materialized clock. SLA semantics are incident-lifetime: REOPEN clears the operational acknowledgement timestamp and the previous resolution capture, but preserves the immutable first-ACK elapsed capture; resolution continues from the original creation epoch through the final resolution. A later acknowledgement restores operational state without replacing first-ACK SLA truth. Accumulated pauses continue shifting the lifetime resolution deadline.
