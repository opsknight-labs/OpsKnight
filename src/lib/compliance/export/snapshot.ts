...
export async function buildAuditSnapshot(
  ...
  const executeSnapshot = async (
    db: Pick<PrismaClient, 'complianceControlState' | 'complianceEvaluation'>
  ): Promise<AuditSnapshotData> => {
    ...
    const states = await db.complianceControlState.findMany({
    ...
    if (controlsNeedingHistoricalEval.length > 0) {
      for (const cid of controlsNeedingHistoricalEval) {
        const histEval = await db.complianceEvaluation.findFirst({
          where: {
            controlId: cid,
    ...
    const evaluations =
      validCurrentEvalIds.length > 0
        ? await db.complianceEvaluation.findMany({
            where: {
  ...
  if ('$transaction' in prisma && typeof (prisma as any).$transaction === 'function') {
    return await (prisma as any).$transaction(
      async (tx: Pick<PrismaClient, 'complianceControlState' | 'complianceEvaluation'>) =>
        executeSnapshot(tx),
