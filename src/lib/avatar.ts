 ...
 * Uses DiceBear's 'lorelei' (sleek minimalist vector portraits) via our local API proxy.
 */
export const getDefaultAvatar = (
  ...
  switch (genderLower) {
    case 'male':
      return `/api/avatar?style=lorelei&seed=${encodeURIComponent(userId)}-male&backgroundColor=${bg}&radius=0`;
    case 'female':
      return `/api/avatar?style=lorelei&seed=${encodeURIComponent(userId)}-female&backgroundColor=${bg}&radius=0`;
    case 'non-binary':
      return `/api/avatar?style=lorelei&seed=${encodeURIComponent(userId)}-nb&backgroundColor=${bg}&radius=0`;
    case 'other':
      return `/api/avatar?style=lorelei&seed=${encodeURIComponent(userId)}-other&backgroundColor=${bg}&radius=0`;
    case 'prefer-not-to-say':
      return `/api/avatar?style=lorelei&seed=${encodeURIComponent(userId)}-neutral&backgroundColor=${bg}&radius=0`;
    default:
      return `/api/avatar?style=lorelei&seed=${encodeURIComponent(userId)}&backgroundColor=${bg}&radius=0`;
  }
};

/**
 * Checks if an avatar URL is one of our default system-generated ones.
 * Returns true for default system avatars (lorelei, legacy big-smile, legacy bottts).
