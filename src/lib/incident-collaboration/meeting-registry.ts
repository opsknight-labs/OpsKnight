...
export class TeamsMeetingAdapter implements MeetingProviderAdapter {
  ...
  async createOrGetMeeting(input: CreateOrGetMeetingInput): Promise<MeetingResult> {
    ...
    // Graph createOrGet endpoint
    const graphEndpoint = 'https://graph.microsoft.com/v1.0/me/onlineMeetings/createOrGet';
    const payload = {
      externalId,
      subject,
    ...
    if (!res.ok) {
      ...
      if (res.status === 401 || res.status === 403) {
        throw new Error(
          `Microsoft Graph returned ${res.status}: Ensure 'OnlineMeetings.ReadWrite.All' application permission is granted in Microsoft Entra admin center.`
        );
      }
      throw new Error(
    ...
    return {
      ...
      organizerEmail:
        data.participants?.organizer?.email || data.participants?.organizer?.upn || null,
      metadata: {
        rawId: data.id,
      },
    };
 ...
 * Enterprise conference link via configured template or vanity room.
 */
export class ZoomMeetingAdapter implements MeetingProviderAdapter {
  ...
  async createOrGetMeeting(input: CreateOrGetMeetingInput): Promise<MeetingResult> {
    ...
    const externalId = `opsknight:${input.incidentId}:${generation}`;
    const joinUrl =
      generateBridgeUrl(input.incidentId, 'ZOOM', input.customTemplate) ||
      `https://zoom.us/my/opsknight-inc-${input.incidentId.slice(-8)}`;
...
/**
 * Google Meet Adapter
 * Workspace room code or lookup link.
 */
export class GoogleMeetAdapter implements MeetingProviderAdapter {
  ...
  async createOrGetMeeting(input: CreateOrGetMeetingInput): Promise<MeetingResult> {
    ...
    const externalId = `opsknight:${input.incidentId}:${generation}`;
    const joinUrl =
      generateBridgeUrl(input.incidentId, 'GOOGLE_MEET', input.customTemplate) ||
      `https://meet.google.com/lookup/opsknight-inc-${input.incidentId.slice(-8)}`;
