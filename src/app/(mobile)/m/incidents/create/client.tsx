...
const CONTROL =
  'min-h-[44px] h-[44px] w-full rounded-xl border border-input bg-background px-3 text-sm text-foreground shadow-sm outline-none transition-colors focus:border-ring focus:ring-2 focus:ring-ring/20';
  ...
  useEffect(() => {
    ...
    void readCache<Draft>(DRAFT_KEY, DRAFT_MAX_AGE_MS).then(draft => {
      ...
      setSelectedServiceId(
        services.some(service => service.id === draft.selectedServiceId)
  ...
  return (
    ...
    >
      {offline ? (
        <div
          role="status"
          className="flex items-start gap-2 rounded-xl border border-amber-300/70 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-900 dark:border-amber-900/70 dark:bg-amber-950/40 dark:text-amber-200"
        >
          <WifiOff className="mt-0.5 h-4 w-4 shrink-0" aria-hidden="true" />
          <span>
            Offline — editing is safe and the draft is saved, but incident creation requires a
            connection.
          </span>
        </div>
      ) : null}
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            className={cn(CONTROL, 'min-h-28 h-auto resize-y py-3')}
            maxLength={10_000}
          />
        </div>
        <div className="space-y-2">
  ...
  return (
    <label
      className={cn(
        ...
        checked ? tone : 'border-input bg-background text-muted-foreground hover:bg-accent'
      )}
    >
      <input
        type="radio"
...
function VisibilityRadio({
  ...
  return (
    <label
      className={cn(
        'flex min-h-11 cursor-pointer items-center justify-center gap-1.5 rounded-xl border px-2 text-xs font-semibold transition-colors',
        checked
    ...
    >
      ...
      />
      <span>{label}</span>
      {isDefault ? <span className="text-[9px] font-medium opacity-70">default</span> : null}
    </label>
  );
}
