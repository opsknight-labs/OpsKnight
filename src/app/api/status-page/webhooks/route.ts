  ...
  try {
    ...
    if (!statusPageId) {
      return jsonError(
        new AppError({
          code: 'VALIDATION_FAILED',
          userMessage: 'statusPageId is required for multi-page isolation',
          fields: [
            { field: 'statusPageId', code: 'required', message: 'statusPageId is required' },
          ],
        })
    ...
    // Validate ownership - ensure webhook belongs to the specified status page
    const existingWebhook = await prisma.statusPageWebhook.findUnique({ where: { id } });
    if (!existingWebhook || existingWebhook.statusPageId !== statusPageId) {
      return jsonError(
        new AppError({
    ...
    logger.info('api.status_page.webhook.updated', { webhookId: id, statusPageId });
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    logger.info('api.status_page.webhook.deleted', { webhookId: id, statusPageId });
