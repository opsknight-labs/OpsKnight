...
function buildSimplePdf(lines: string[]): Buffer {
  ...
  const objects: { id: number; data: string }[] = [
    ...
    { id: 3, data: '<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>' },
  ];
  ...
  pages.forEach((pageLines, idx) => {
    ...
    objects.push({
      id: contentObjId,
      data: `<< /Length ${Buffer.byteLength(streamContent, 'utf8')} >>\nstream\n${streamContent}\nendstream`,
    });
  });
...
export async function GET(req: NextRequest) {
  ...
  if (!isAdmin) {
    ...
    if (!publicPage) return new NextResponse('Unauthorized', { status: 403 });
  ...
  try {
    ...
    if (isNaN(monthIndex) || monthIndex < 0 || monthIndex > 11) monthIndex = now.getUTCMonth();
    ...
    const statusPage = await prisma.statusPage.findFirst({
      where: { enabled: true },
      include: {
        services: { include: { service: true }, where: { showOnPage: true } },
      },
    });

    if (!statusPage) return new NextResponse('Status page not found', { status: 404 });
    if (!statusPage.enableUptimeExports) return new NextResponse('Uptime exports are disabled', { status: 403 });

    const serviceIds = statusPage.services.map(sp => sp.serviceId);
    if (serviceIds.length === 0) return new NextResponse('No services configured', { status: 400 });
