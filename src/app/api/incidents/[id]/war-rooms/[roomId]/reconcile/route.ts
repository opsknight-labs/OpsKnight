...
export async function POST(
  ...
  try {
    ...
    const actor = await assertCanManageWarRoom(id);
    const room = await prisma.incidentWarRoom.findFirst({
      where: { id: roomId, incidentId: id },
      select: { provider: true, state: true, createAttemptedAt: true },
    });
    if (!room)
      return jsonError(
        new AppError({ code: 'RESOURCE_NOT_FOUND', userMessage: 'War room not found.' })
      );
    ...
    const isReconcilable =
      room.state === 'READY' || (room.state === 'AMBIGUOUS' && Boolean(room.createAttemptedAt));
    if (!isReconcilable)
      return jsonError(
        new AppError({
          code: 'VALIDATION_FAILED',
          userMessage:
            'Only an ambiguous war room with a recorded create attempt or a ready war room can be reconciled.',
        })
      );
    ...
    if (!queued)
      return jsonError(
        new AppError({
          code: 'VALIDATION_FAILED',
          userMessage: 'Unable to queue war-room reconciliation.',
        })
      );
    await emitAuditEvent({
      action: 'INCIDENT_WAR_ROOM_RECONCILIATION_REQUESTED',
      source: 'UI',
      target: { type: 'INCIDENT', id },
      actor: { type: 'USER', id: actor.id },
      metadata: { provider: room.provider, warRoomId: roomId, mode: 'MARKER_ONLY' },
    });
