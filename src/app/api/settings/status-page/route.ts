...
export async function POST(req: NextRequest) {
  try {
    ...
    // Require explicit statusPageId for updates - no implicit default fallback
    if (!statusPageId) {
      return jsonError(
        new AppError({
          code: 'VALIDATION_FAILED',
          userMessage: 'Status page ID is required.',
          fields: [
            {
              field: 'id',
              code: 'required',
              message: 'Status page ID is required for updates.',
            },
          ],
        })
      );
    ...
    if (!existingPage) {
      return jsonError(
        new AppError({
          ...
          userMessage: 'Status page not found.',
        }),
        404
      );
    }
    ...
    if (name !== undefined && name !== null && name.trim().length > 0) {
      try {
        const uniqueName = await assertStatusPageNameAvailable(name, { excludeId: statusPageId });
    ...
    if (Array.isArray(serviceIds)) {
      await prisma.statusPageService.deleteMany({ where: { statusPageId } });
    ...
    logger.info('api.status_page.updated', { statusPageId });
