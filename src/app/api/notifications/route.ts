...
export async function GET(req: NextRequest) {
  try {
    ...
    const where = unreadOnly ? { ...baseWhere, readAt: null } : baseWhere;
    ...
    const formattedNotifications = notifications.map(notification => {
      ...
      const incidentId = notification.entityType === 'INCIDENT' ? notification.entityId : null;
    ...
    logger.error('api.notifications.fetch_error', {
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (error instanceof AppError) return jsonError(error);
    logger.error('api.notifications.update_error', {
      error: error instanceof Error ? error.message : String(error),
    });
