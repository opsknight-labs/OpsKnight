    ...
    try {
      ...
      try {
        const response = await fetch(
          `/api/auth/oidc/logout-url?callbackUrl=${encodeURIComponent(callbackUrl)}`,
          { cache: 'no-store' }
        );
        if (response.ok) {
          const payload = (await response.json()) as { url?: unknown };
          providerLogoutUrl = typeof payload.url === 'string' ? payload.url : null;
        }
      } catch {
        // Provider logout is best-effort; local logout remains authoritative.
      }
      try {
      ...
      window.location.assign(providerLogoutUrl ?? callbackUrl);
    } catch {
      ...
      <AuthCard>
        <div className="mb-8 text-center">
          ...
          <p className="mt-1.5 text-sm text-slate-500 dark:text-slate-400">
            End your current OpsKnight session.
          </p>
        </div>
        {error && (
          <div
            role="alert"
            className="mb-4 flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700 dark:border-red-500/20 dark:bg-red-500/10 dark:text-red-300"
          >
        ...
        <div className="space-y-3">
          ...
          >
            {isSigningOut ? (
              <>
                <Spinner size="sm" variant="current" /> Signing out…
              </>
            ) : (
              <>
