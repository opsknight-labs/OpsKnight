...
import React, { useMemo, useTransition, type ReactNode } from 'react';
...
export default function DetailTabs({
  ...
  return (
    <Tabs value={currentTab} onValueChange={handleTabChange} className={cn('space-y-6', className)}>
          ...
          >
            {tabs.map(tab => (
              <TabsTrigger
                ...
                className={cn(
                  'group relative flex items-center justify-center gap-2 rounded-lg py-2.5 px-3.5 text-xs font-medium transition-all duration-150',
      ...
      {/* Render children or tab contents */}
      {children
        ? children
        : tabs.map(tab =>
            tab.content ? (
              <TabsContent
                key={tab.id}
                value={tab.id}
                className={cn('focus-visible:outline-hidden', contentClassName)}
              >
                {tab.content}
              </TabsContent>
            ) : null
