...
export default function TeamLeadBadge({ lead, className, size = 'sm' }: TeamLeadBadgeProps) {
  if (!lead) {
    return (
      <Badge
        ...
        size="xs"
        className={cn(
          'text-[10px] font-medium text-muted-foreground/80 border-dashed bg-muted/20 shrink-0',
          className
        )}
      >
        No lead
  ...
  return (
    <div
      className={cn(
        'inline-flex items-center gap-1.5 rounded-full border border-border/80 bg-card/80 py-0.5 pl-0.5 pr-2 shrink-0 shadow-2xs',
    ...
    >
      ...
      <span className="flex items-center gap-0.5 text-[9px] font-bold uppercase tracking-wider text-primary bg-primary/10 px-1 py-0.5 rounded ring-1 ring-inset ring-primary/20 shrink-0">
